...
// Only wire these up if running inside Electron (window.windowControls exists).
// On a plain website there's no OS window to minimize/close, so hide the buttons instead.
if (window.windowControls) {
  ...
  document.getElementById('btn-min').classList.add('hidden');
...
// ---------- Scene navigation ----------
const ORDER = ['scene-0','scene-1','scene-1b','scene-2','scene-3','scene-4','scene-4b','scene-5','scene-6','scene-7'];
let historyStack = [];
let current = 'scene-0';
...
function refreshNameText() {
  ...
  const cardGreet = document.getElementById('card-greet');
  if (cardGreet) cardGreet.textContent = `Happy birthday ${userName},`;
}
...
document.getElementById('btn-replay').addEventListener('click', () => {
  ...
  document.getElementById('btn-open-card').classList.add('hidden');
  // music keeps playing straight through the replay, it is never paused/reset here
...
function typeLetter() {
  ...
  const interval = setInterval(() => {
    ...
    if (i >= text.length) {
      ...
      document.getElementById('btn-open-card').classList.remove('hidden');
    }
  }, 28);
}
